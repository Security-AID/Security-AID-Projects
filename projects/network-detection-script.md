...
Every published screenshot includes a short explanation of what is being shown. Screenshots that displayed an access password or target value were redacted before publication.
...
### Ref 3: Detector Menu and Sniff Traffic Option

![Detector tool menu with sensitive values redacted](images/network-detection/network-detector-03.png)

This screenshot shows the DETECTOR program menu after access was granted. The menu includes options for SYN scanning, UDP scanning, web exploit detection, traffic sniffing, URL scanning, and an Nmap-like port scanner. The access password and target value were redacted before publishing.

---

### Ref 4: Traffic Sniffer Menu on My Own Wi-Fi

![Detector traffic sniffer menu with sensitive values redacted](images/network-detection/network-detector-04.png)

This screenshot shows the traffic-sniffer workflow being selected on my own Wi-Fi environment. The sniffer menu provides an option to start traffic capture or return to the main menu, while sensitive input values are redacted for safe public documentation.
...
- Documented the tool's output while redacting screenshots that exposed an access password or target value.
